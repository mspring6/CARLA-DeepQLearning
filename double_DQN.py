...
class DQNAgent:
    ...
    def create_model(self):
        ...
        model.compile(loss="mse", optimizer=Adam(learning_rate=0.00001), metrics=["accuracy"])                                 ## changed
        return model
...
class ModifiedTensorBoard(TensorBoard):
    ...
    def __init__(self, **kwargs):
        ...
        self.writer = tf.summary.create_file_writer('doubleDQN-log')
...
if __name__ == '__main__':
    ...
    #Need to setup both Q networks separatley 
    agent1 = DQNAgent()
    agent2 = DQNAgent()
    env = CarEnv()
    ...
    trainer_thread1 = Thread(target=agent1.train_in_loop, daemon=True)
    trainer_thread2 = Thread(target=agent2.train_in_loop, daemon=True)
    trainer_thread1.start()
    trainer_thread2.start()
    while not (agent1.training_initialized or agent2.training_initialized):
        time.sleep(0.01)

    ## 
    agent1.get_qs(np.ones((env.im_height, env.im_width, 3)))
    agent2.get_qs(np.ones((env.im_height, env.im_width, 3)))
    ...
    for episode in tqdm(range(1, EPISODES + 1), unit='episodes'):
        #try:
            ...
            while True:
                ...
                if pick == 1:
                    # Get action from Q2 table
                    action = np.argmax(agent2.get_qs(current_state))
                else:
                    # Get action from Q1 table
                    action = np.argmax(agent1.get_qs(current_state))
            ...
            if not episode % AGGREGATE_STATS_EVERY or episode == 1:        ## every show_stats_every, which is 10 right now, show and save teh following
                ...
                if average_reward >= -100:
                    agent1.model.save('models/Double_DQNmodel_Q1')
                    agent2.model.save('models/Double_DQNmodel_Q2')
    ...
    agent1.terminate = True
    agent2.terminate = True
    trainer_thread1.join()
    trainer_thread2.join()
    agent1.model.save('models/Double_DQNmodel_Q1')
    agent2.model.save('models/Double_DQNmodel_Q2')