...
#Header for any CARLA script, this is for the CARLA python library 
try:
...
#Class for the extended actions
class ACTIONS:
    forward = 0
    left = 1
    right = 2
...
#Class to create the Car Environment
class CarEnv:
    ...
    def __init__(self):
        self.client = carla.Client('localhost', 2000) #This is what listens to the server
        self.client.set_timeout(200.0)
        # self.actor = carla.Actor
        #self.world = self.client.load_world('Town03')
        #self.map = self.world.get_map()   ## added for map creating
        self.world = self.client.get_world() #Get the blueprint for the world, i.e. the map
        self.blueprint_library = self.world.get_blueprint_library()

        self.model_3 = self.blueprint_library.filter("model3")[0]  #tesla model3 from library
        
        self.actions = [getattr(ACTIONS, action) for action in ACTION] #Get the number of actions from the action class
    ...
    def collision_data(self, event):
        self.collision_hist.append(event) #Look to see if a collision occured 
        
    def lane_invasion_data(self, event):
        #Look to see if a lane invasion occured 
        self.laneInvasion_hist.append(event)
        

    def process_img(self, image):
        #Function to process the image to be passed to the CNN
    ...
    def step(self, action):
        #Function to apply control to the vehicle and get the reward, i.e. this is the function that takes the action 
...
class DQNAgent:
    ...
    def create_model(self):
        ...
        x = base_model.output
        x = AveragePooling2D(pool_size=(15,30))(x)
        x = Conv2D(filters = 1000, kernel_size=(1,1), activation = 'relu')(x)
        x = Flatten()(x) 

        predictions = Dense(8, activation="linear")(x)  ## output layer include 8 nuros, representing 8 actions
    ...
    def train(self):
        ...
        #Filepath
        filepath = 'models/Improved_DQNmodel'
        #Checkpoint
...
#Class that saves the results using a library called TensorBoard
class ModifiedTensorBoard(TensorBoard):
    ...
    def __init__(self, **kwargs):
        ...
        self.writer = tf.summary.create_file_writer('ImprovedDQN-log')
...
if __name__ == '__main__':
    ...
    #Get the Q value from the Q function 
        ...
        try:
            ...
            if not episode % AGGREGATE_STATS_EVERY or episode == 1:        ## every show_stats_every, which is 10 right now, show and save teh following
                ...
                if average_reward >= MIN_REWARD:
                    agent.model.save('Improved_DQNmodel')
            ...
            trainer_thread.join()
            agent.model.save('models/Improved_DQNmodel')
